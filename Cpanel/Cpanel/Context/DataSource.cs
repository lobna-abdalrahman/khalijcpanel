...
//                result = cmd.ExecuteNonQuery();
            ...
            using (OracleConnection con = new OracleConnection(conString))
            {
                ...
                try
                {
                    ...
                    if (dr1.Read())
                    {
                        ...
                        name = dr1[1].ToString();
                        cmd = new OracleCommand("update users set user_pwd='" + enc_pwd.ToString() + "' , first_login='T',USER_PAS='" + re.ToString() + "' where  def_acc='" + account + "'", con);
                        cmd.ExecuteNonQuery();
                        lblconfirm = "Successfully";
        ...
        public custinfo getcustinfo(String branchcode, String acttype, String acc_no, String acc_curr, String category)
        {
            ...
            String query1 = "select  u.user_id, u.user_name,u.user_log,u.user_pwd,u.user_email,u.user_mobile,u.user_adrs,m.name,u.user_status" +
             " from users u, tbl_rolemaster m  where u.roleid=m.roleid and u.DEF_ACC='99" + branchcode + acttype + acc_curr + acc_no + "' and  catogry ='"+category+"'";
